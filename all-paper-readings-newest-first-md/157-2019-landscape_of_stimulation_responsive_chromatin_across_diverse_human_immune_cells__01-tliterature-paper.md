# 094. Landscape of stimulation-responsive chromatin across diverse human immune cells
...
- PMID：31548721
- 主题：bulk immune ATAC/RNA atlas；stimulation response；human immune genetics；GWAS annotation

## 为什么重要
不是单细胞方法论文，但提供 population immune regulatory annotation，可作为 T-cell stimulation priors and variant-to-state mapping reference。 对“单细胞组学 × T 细胞 × 人群免疫力”方法报告而言，这篇文献主要用于回答两个问题：第一，现有技术或算法已经能把哪些免疫信息带入单细胞分析；第二，哪些层次仍停留在后处理统计，适合进一步发展新算法。

## 数据与研究设计
- 数据性质：Human PBMC-derived/sorted immune cell populations and fetal thymus cell types；resting and stimulated conditions for up to 32 immune cell populations；GEO ATAC-seq GSE118189，RNA-seq GSE118165，ChIP-seq GSE126505；portal at Stanford/Pritchard lab。
...
## 文章中的算法/分析流程
### 1. 输入层
sorted-cell ATAC-seq/RNA-seq atlas, differential accessibility, TF footprint/motif analysis, stimulation-responsive regulatory region annotation and GWAS overlap。 输入通常是 cell barcode 对齐的 count/feature matrix，或 sorted-cell epigenomic tracks/peak matrices，再叠加 sample、condition、stimulation、cell type、donor 等 metadata。
...
## 代码输入、输出、模型结构和意义
- 代码/工具：No single standalone algorithm package is central；data portal and supplementary resources support genome-browser-level reuse。
...
## 对新算法贡献程度
- 直接算法贡献：中到高，取决于条目；094 的核心贡献可概括为 `sorted-cell ATAC-seq/RNA-seq atlas, differential accessibility, TF footprint/motif analysis, stimulation-responsive regulatory region annotation and GWAS overlap。`。
...
## 数据可用性
- DOI：https://doi.org/10.1038/s41588-019-0505-9
- 公开数据/入口：Human PBMC-derived/sorted immune cell populations and fetal thymus cell types；resting and stimulated conditions for up to 32 immune cell populations；GEO ATAC-seq GSE118189，RNA-seq GSE118165，ChIP-seq GSE126505；portal at Stanford/Pritchard lab。
- 代码/工具：No single standalone algorithm package is central；data portal and supplementary resources support genome-browser-level reuse。
...
## 一句话结论
094 是 81-94 这一段方法文献中的关键节点：它把 `bulk immune ATAC/RNA atlas；stimulation response；human immune genetics；GWAS annotation` 转化为可计算、可复用或可作为 benchmark 的单细胞免疫分析问题。
