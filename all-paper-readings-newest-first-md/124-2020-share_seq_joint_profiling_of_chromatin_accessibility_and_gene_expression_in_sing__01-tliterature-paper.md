# 090. Chromatin potential identified by shared single-cell profiling of RNA and chromatin
...
- PMID：33098772
- 主题：SHARE-seq；joint RNA+ATAC；chromatin potential；regulatory program inference

## 为什么重要
虽然不是 T-cell 专文，但其 chromatin-potential 思想可迁移到 T cell activation/exhaustion 的 pre-state prediction。 对“单细胞组学 × T 细胞 × 人群免疫力”方法报告而言，这篇文献主要用于回答两个问题：第一，现有技术或算法已经能把哪些免疫信息带入单细胞分析；第二，哪些层次仍停留在后处理统计，适合进一步发展新算法。

## 数据与研究设计
- 数据性质：Homo sapiens cell lines and Mus musculus skin/brain/lung；GEO GSE140203；BioProject PRJNA588784；SRA SRP229493；processed data and Shiny skin network portal。
...
## 文章中的算法/分析流程
### 1. 输入层
combinatorial indexing joint RNA and ATAC profiling；correlates chromatin accessibility with later RNA expression to define chromatin potential。 输入通常是 cell barcode 对齐的 count/feature matrix，或 sorted-cell epigenomic tracks/peak matrices，再叠加 sample、condition、stimulation、cell type、donor 等 metadata。
...
## 代码输入、输出、模型结构和意义
- 代码/工具：Data portal https://buenrostrolab.shinyapps.io/skinnetwork/；analysis built around paired RNA/ATAC matrices and regulatory program inference。
...
## 对新算法贡献程度
- 直接算法贡献：中到高，取决于条目；090 的核心贡献可概括为 `combinatorial indexing joint RNA and ATAC profiling；correlates chromatin accessibility with later RNA expression to define chromatin potential。`。
...
## 数据可用性
- DOI：https://doi.org/10.1016/j.cell.2020.09.056
- 公开数据/入口：Homo sapiens cell lines and Mus musculus skin/brain/lung；GEO GSE140203；BioProject PRJNA588784；SRA SRP229493；processed data and Shiny skin network portal。
- 代码/工具：Data portal https://buenrostrolab.shinyapps.io/skinnetwork/；analysis built around paired RNA/ATAC matrices and regulatory program inference。
...
## 一句话结论
090 是 81-94 这一段方法文献中的关键节点：它把 `SHARE-seq；joint RNA+ATAC；chromatin potential；regulatory program inference` 转化为可计算、可复用或可作为 benchmark 的单细胞免疫分析问题。
