# 093. Chromatin activity at GWAS loci identifies T cell states driving complex immune diseases

## 基本信息
- 年份：2019
- 期刊：Nature Genetics
- DOI：https://doi.org/10.1038/s41588-019-0493-9
- PMID：31548716
- 主题：CHEERS；GWAS enrichment；T cell/macrophage activation states；immune disease genetics

## 为什么重要
这是 81-94 中最直接的统计方法贡献之一：把 GWAS SNP enrichment 从二值 peak overlap 推向 quantitative state-specific chromatin activity。 对“单细胞组学 × T 细胞 × 人群免疫力”方法报告而言，这篇文献主要用于回答两个问题：第一，现有技术或算法已经能把哪些免疫信息带入单细胞分析；第二，哪些层次仍停留在后处理统计，适合进一步发展新算法。

## 数据与研究设计
- 数据性质：Human T cells and macrophages stimulated with disease-relevant cytokines；raw sequencing in EGA：ATAC-seq EGAS00001003501，H3K27ac EGAS00001002749。
...
## 文章中的算法/分析流程
### 1. 输入层
CHEERS models subtle quantitative chromatin-state changes and tests enrichment of disease-associated SNPs across immune cell states。 输入通常是 cell barcode 对齐的 count/feature matrix，或 sorted-cell epigenomic tracks/peak matrices，再叠加 sample、condition、stimulation、cell type、donor 等 metadata。
...
## 代码输入、输出、模型结构和意义
- 代码/工具：CHEERS GitHub：https://github.com/trynkaLab/CHEERS
...
## 对新算法贡献程度
- 直接算法贡献：中到高，取决于条目；093 的核心贡献可概括为 `CHEERS models subtle quantitative chromatin-state changes and tests enrichment of disease-associated SNPs across immune cell states。`。
...
## 数据可用性
- DOI：https://doi.org/10.1038/s41588-019-0493-9
- 公开数据/入口：Human T cells and macrophages stimulated with disease-relevant cytokines；raw sequencing in EGA：ATAC-seq EGAS00001003501，H3K27ac EGAS00001002749。
- 代码/工具：CHEERS GitHub：https://github.com/trynkaLab/CHEERS
...
## 一句话结论
093 是 81-94 这一段方法文献中的关键节点：它把 `CHEERS；GWAS enrichment；T cell/macrophage activation states；immune disease genetics` 转化为可计算、可复用或可作为 benchmark 的单细胞免疫分析问题。
