# 085. Cell hashing with barcoded antibodies enables multiplexing and doublet detection for single cell genomics

## 基本信息
- 年份：2018
- 期刊：Genome Biology
- DOI：https://doi.org/10.1186/s13059-018-1603-1
- PMID：30567574
- 主题：sample multiplexing；HTO demultiplexing；doublet detection；cohort-scale scRNA-seq

## 为什么重要
该方法把样本混池和 doublet 检出变成单细胞队列设计的一部分，直接降低人群免疫队列的批次和成本压力。 对“单细胞组学 × T 细胞 × 人群免疫力”方法报告而言，这篇文献主要用于回答两个问题：第一，现有技术或算法已经能把哪些免疫信息带入单细胞分析；第二，哪些层次仍停留在后处理统计，适合进一步发展新算法。

## 数据与研究设计
- 数据性质：Homo sapiens immune/cell-line droplet scRNA-seq；8 个 hashtag antibody pools；GEO GSE108313；BioProject PRJNA423077；SRA SRP127179。
...
## 文章中的算法/分析流程
### 1. 输入层
HTO count matrix normalization + positive/negative signal separation + sample assignment + cross-sample doublet detection 输入通常是 cell barcode 对齐的 count/feature matrix，或 sorted-cell epigenomic tracks/peak matrices，再叠加 sample、condition、stimulation、cell type、donor 等 metadata。
...
## 代码输入、输出、模型结构和意义
- 代码/工具：Seurat HTO demultiplexing workflow；https://github.com/satijalab/seurat
...
## 对新算法贡献程度
- 直接算法贡献：中到高，取决于条目；085 的核心贡献可概括为 `HTO count matrix normalization + positive/negative signal separation + sample assignment + cross-sample doublet detection`。
...
## 数据可用性
- DOI：https://doi.org/10.1186/s13059-018-1603-1
- 公开数据/入口：Homo sapiens immune/cell-line droplet scRNA-seq；8 个 hashtag antibody pools；GEO GSE108313；BioProject PRJNA423077；SRA SRP127179。
- 代码/工具：Seurat HTO demultiplexing workflow；https://github.com/satijalab/seurat
...
## 一句话结论
085 是 81-94 这一段方法文献中的关键节点：它把 `sample multiplexing；HTO demultiplexing；doublet detection；cohort-scale scRNA-seq` 转化为可计算、可复用或可作为 benchmark 的单细胞免疫分析问题。
