# 086. MULTI-seq: sample multiplexing for single-cell RNA sequencing using lipid-tagged indices
...
- PMID：31209384
- 主题：lipid-tagged sample barcoding；multiplexing；doublet detection；large-cohort design

## 为什么重要
比抗体依赖的 hashing 更通用，适合没有稳定 pan-cell surface marker 或跨物种/组织样本的 cohort multiplexing。 对“单细胞组学 × T 细胞 × 人群免疫力”方法报告而言，这篇文献主要用于回答两个问题：第一，现有技术或算法已经能把哪些免疫信息带入单细胞分析；第二，哪些层次仍停留在后处理统计，适合进一步发展新算法。

## 数据与研究设计
- 数据性质：人和鼠细胞/组织示例；Nature 论文页显示 mouse immune-cell 示例 n=10,427 cells；原文提供 source data 和 supplementary protocol。
...
## 文章中的算法/分析流程
### 1. 输入层
lipid-modified oligo barcode labeling + barcode UMI matrix thresholding/classification + singlet/doublet/negative calls 输入通常是 cell barcode 对齐的 count/feature matrix，或 sorted-cell epigenomic tracks/peak matrices，再叠加 sample、condition、stimulation、cell type、donor 等 metadata。
...
## 代码输入、输出、模型结构和意义
- 代码/工具：MULTI-seq demultiplexing scripts/workflow commonly distributed through Gartner lab/GitHub resources；算法可复现于 barcode count matrix。
...
## 对新算法贡献程度
- 直接算法贡献：中到高，取决于条目；086 的核心贡献可概括为 `lipid-modified oligo barcode labeling + barcode UMI matrix thresholding/classification + singlet/doublet/negative calls`。
...
## 数据可用性
- DOI：https://doi.org/10.1038/s41592-019-0433-8
- 公开数据/入口：人和鼠细胞/组织示例；Nature 论文页显示 mouse immune-cell 示例 n=10,427 cells；原文提供 source data 和 supplementary protocol。
- 代码/工具：MULTI-seq demultiplexing scripts/workflow commonly distributed through Gartner lab/GitHub resources；算法可复现于 barcode count matrix。
...
## 一句话结论
086 是 81-94 这一段方法文献中的关键节点：它把 `lipid-tagged sample barcoding；multiplexing；doublet detection；large-cohort design` 转化为可计算、可复用或可作为 benchmark 的单细胞免疫分析问题。
