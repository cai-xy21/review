...
- PMID：30567574
- Code/Tool：Seurat HTO demultiplexing workflow；https://github.com/satijalab/seurat

## 算法视角定位
本文的算法定位是：HTO count matrix normalization + positive/negative signal separation + sample assignment + cross-sample doublet detection。在 81-94 这一组文献中，它用于补齐 TCR/BCR tooling、sample multiplexing、多模态测量、RNA/ATAC regulatory inference 或 GWAS-to-cell-state enrichment 的方法拼图。

## 输入与输出
- 输入：Homo sapiens immune/cell-line droplet scRNA-seq；8 个 hashtag antibody pools；GEO GSE108313；BioProject PRJNA423077；SRA SRP127179。
...
### 2. 关键计算模块
HTO count matrix normalization + positive/negative signal separation + sample assignment + cross-sample doublet detection 这是该条目最值得在 method report 中展开的算法或 workflow 层贡献。
...
## 数据可用性评估
- DOI 和 accession：https://doi.org/10.1186/s13059-018-1603-1；Homo sapiens immune/cell-line droplet scRNA-seq；8 个 hashtag antibody pools；GEO GSE108313；BioProject PRJNA423077；SRA SRP127179。
- 代码：Seurat HTO demultiplexing workflow；https://github.com/satijalab/seurat
